...
where
    ...
    Op: Fn(T, T) -> T + Copy,
{
    pub fn new(size: usize, func: Op) -> SegmentTree<T, Op> {
        let size = size.next_power_of_two();
        SegmentTree {
            size,
            vec: vec![None; size * 2 - 1],
            func,
        }
    }

    // pub fn fill(&mut self, items: &[T]) {
    pub fn fill<I>(&mut self, items: I)
    where
        I: IntoIterator<Item = T>,
    {
        let idx = self.size - 1;
        for (entry, item) in self.vec.iter_mut().skip(idx).zip(items) {
            *entry = Some(item);
        }
    ...
    fn update_single(&mut self, k: usize) {
        let left = self.vec[k * 2 + 1];
        let right = self.vec[k * 2 + 2];
        self.vec[k] = self.op(left, right);
    }
    ...
    pub fn query<R: std::ops::RangeBounds<usize>>(&self, range: R) -> Option<T> {
        ...
        self.query_range(start..end, 0, 0..self.size)
    }
